...
class DataTrainingArguments:
    ...
    output_file: Optional[str] = field(
        default=None,
        metadata={"help": "the name of your output file."},
    )
...
def main():
    ...
    if training_args.do_predict:
        ...
        for i, pred in enumerate(preds):
            tmp = []
            tmp.append(test_dataset['video-id'][i])
            tmp.append(pred)
            """
            ex = {
                'id': test_dataset['video-id'][i],
                'relevant': pred  if pred < 4  else 0
            }
            
            output_json['data'].append(ex)
            """
            All.append(tmp)
        df = pd.DataFrame(All, columns = ["id","relevant"])
        df.to_csv(data_args.output_file)
